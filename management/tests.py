...
class ParkingTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.parking_space = ParkingSpace.objects.create(cod="A01", vacancy_type="conventional", created_by=self.user,status=True, parking=self.parking)
    ...
    def test_list_parking_spaces_for_parking(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.token.key}')
        url = reverse('list-parking-spaces', kwargs={'pk': self.parking.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TicketTestCase(APITestCase):
    ...
    def test_cannot_create_ticket_for_priority_parking_space(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.token.key}')
        
        # Criar uma ParkingSpace com vacancy_type 'priority'
        priority_parking_space = ParkingSpace.objects.create(
            cod="B01",
            status=False,
            parking=self.parking,
            vacancy_type='priority',  # Defina o tipo de vaga como 'priority'
            created_by=self.user
        )
        ...
        data = {
            ...
            "checkin": "09:00:00",
            "checkout": "00:00:00",
            "parking_space": priority_parking_space.id,  # Passar o ID da vaga de estacionamento
            "value": 0.0
        }
        
        response = self.client.post(url, data)
        
        # Verificar se a resposta é 400 Bad Request devido à validação
...
class ReservationTestCase(APITestCase):
    ...
    def test_create_reservation(self):
        ...
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Reservation.objects.filter(parking_space=self.parking_space).exists())
    ...
    def test_update_reservation(self):
        ...
        self.assertEqual(self.reservation.start_time.strftime("%H:%M:%S"), "09:00:00")
        self.assertEqual(self.reservation.end_time.strftime("%H:%M:%S"), "10:00:00")

    def test_reservation_reading(self):
        url = reverse("reservations-detail", kwargs={'pk': self.reservation.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(Reservation.objects.filter(parking_space=self.parking_space.pk).exists())
